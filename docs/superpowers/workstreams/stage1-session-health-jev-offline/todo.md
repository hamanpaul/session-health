...
- [ ] S1.4 — T04: Integrate explicit offline mode across positional/single/batch CLI and terminal/JSON/HTML; keep every selected session status. Add corresponding documentation and CLI help.
- [ ] S1.5 — Run offline parser, bundle, metrics and renderer unit/integration tests including no-network/no-model entry-point checks; collect RED/GREEN evidence.
- [ ] S1.6 — AGY adversarial review of exact candidate, Luna repairs, then root independent verification. Document live/platform limits.
