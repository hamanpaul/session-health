...
---
# 1-2 Jev typed seven-axis batch enhancement
...
Evaluation boundary: developer-authored synthetic expectations are regression fixtures, not human-prelabeled gold. Human label confirmation and held-out evaluation remain root-owned and pending; do not read operator report held_out/proposed-label files or claim pilot accuracy/completion without confirmed human labels. Implement and verify the semantic pipeline independently of that pending evaluation. Read TYPESAFE_API_KEY only from the process environment; if absent, report mock validation separately from live DEFERRED. This preserves the master acceptance requirements.
...
- [ ] S2.4 — Integrate --jev into CLI and all report surfaces while preserving offline facts, raw judgments, coverage, partial failure and no automatic generative analysis.
- [ ] S2.5 — Run synthetic mock E2E and labeled English/Traditional-Chinese pilot fixtures; perform a small non-sensitive live Jev smoke only if the key is visible and record actual model/tokens.
- [ ] S2.6 — AGY adversarial review of exact candidate, Luna repairs, then root independent verification.
